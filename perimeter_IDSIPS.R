...
#+0.25 IPS Operational+25% 
#+0.25 Devices in Support
#+0.25 Events Tuned
df$kpi_value <- as.integer('0')


#If the device is monitored by us, assume for now that it is tuned and only providing IDS
df$kpi_value <- ifelse(df$monitored_by == 'CUSTOMER' && df$device_health_status == 'OK', df$kpi_value + 0.5, df$kpi_value)
